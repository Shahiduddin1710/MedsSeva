...
export const sendForgotPasswordOtpMobile = async (req: Request, res: Response) => {
  try {
    const { mobile } = req.body;
    if (!mobile) return res.status(400).json({ error: 'Mobile number is required' });
    ...
    return res.json({ success: true, message: 'Password reset code sent to your mobile number' });
  } catch (error: any) {
    console.error('sendForgotPasswordOtpMobile error:', error.message);
    res.status(500).json({ error: 'Failed to send reset code. Please try again.' });
  }
};

export const verifyForgotPasswordOtpMobile = async (req: Request, res: Response) => {
  try {
    const { mobile, otp } = req.body;
    if (!mobile || !otp) return res.status(400).json({ error: 'Mobile and OTP are required' });
    ...
    console.error('verifyForgotPasswordOtpMobile error:', error.message);
    res.status(500).json({ error: 'Failed to verify code. Please try again.' });
  }
};
export const resetPasswordMobile = async (req: Request, res: Response) => {
  try {
    ...
    await prisma.user.update({
      where: { mobile },
      data: { password: hashedPassword, otpHash: null, otpExpiresAt: null, otpLastSentAt: null },
    });
